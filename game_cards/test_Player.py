...
class TestPlayer(TestCase):
    ...
    # Test that add_card adding a card to the player list
    def test_valid_add_card(self):
        deck = DeckOfCards()
        card = Card(2, 6)
        self.test_player.set_hand(deck)
        self.assertEqual(len(self.test_player.P_cards), 26)
        self.test_player.add_card(card)
        self.assertEqual(len(self.test_player.P_cards), 27)

    # Test that the add_card adding the specific card to the list
    def test_valid_add_card2(self):
        deck = DeckOfCards()
        card = Card(2, 6)
        self.test_player.set_hand(deck)
        self.assertNotIn(card, self.test_player.P_cards)
        self.test_player.add_card(card)
        self.assertIn(card, self.test_player.P_cards)

    # Test that the card type must be Card in addition to add him
    def test_invalid_add_card(self):
        with self.assertRaises(TypeError):
            invalid_card =  12,"afaf"
            self.test_player.add_card(invalid_card)